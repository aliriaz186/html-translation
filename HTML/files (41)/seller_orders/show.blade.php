    	...
    	<div class="panel-body">
            ...
            <div class="row">
                ...
                <div class="col-lg-3">
                    ...
                    <select class="form-control demo-select2"  data-minimum-results-for-search="Infinity" id="update_delivery_status">
                        ...
                        <option value="delivered" @if ($delivery_status == 'delivered') selected @endif>{{__('Shipped')}}</option>
                        <option value="canceled" @if ($delivery_status == 'canceled') selected @endif>{{__('Canceled')}}</option>
                        <option value="returned" @if ($delivery_status == 'returned') selected @endif>{{__('Returned')}}</option>
    		...
    		<div class="row">
    			<div class="col-lg-12 table-responsive">
    				<table class="table table-bordered invoice-summary">
        				...
        				<tbody>
                            @php
                                $seller_user_id = \App\User::where('id', $order->orderDetails[0]->seller_id)->first()->id;
                            @endphp
                            @foreach ($order->orderDetails->where('seller_id', $seller_user_id) as $key => $orderDetail)
                                <tr>
                                    <td>{{ $key+1 }}</td>
                					<td>
    		...
    		<div class="clearfix">
    			...
    			<tr>
    				...
    				<td>
    					{{ single_price($order->orderDetails->where('seller_id', $seller_user_id)->sum('price')) }}
    				</td>
    			</tr>
    			<tr>
    				<td>
    				...
    				<td>
    					{{ single_price($order->orderDetails->where('seller_id', $seller_user_id)->sum('tax')) }}
    				</td>
    			</tr>
                <tr>
    				<td>
    				...
    				<td>
    					{{ single_price($order->orderDetails->where('seller_id', $seller_user_id)->sum('shipping_cost')) }}
    				</td>
    			</tr>
    			<tr>
    				<td>
    				...
    				<td class="text-bold h4">
    					{{ single_price($order->orderDetails->where('seller_id', $seller_user_id)->sum('price') + $order->orderDetails->where('seller_id', $seller_user_id)->sum('tax') + $order->orderDetails->where('seller_id', $seller_user_id)->sum('shipping_cost')) }}
    				</td>
    			</tr>
    			</tbody>
    			</table>
