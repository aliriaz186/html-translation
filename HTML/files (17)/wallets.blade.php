...
<div class="panel">
    <div class="panel-heading bord-btm clearfix pad-all h-100">
        <h3 class="panel-title pull-left pad-no">{{__('customers wallets')}}</h3>
        <div class="pull-right clearfix">
            <form class="" id="sort_customers" action="" method="GET">
                <div class="box-inline pad-rgt pull-left">
                    <div class="select" style="min-width: 300px;">
                        <select class="form-control demo-select2" name="approved_status" id="approved_status" onchange="sort_customers()">
            ...
            <table class="table table-striped table-responsive mar-no" cellspacing="0" width="100%">
                ...
                <tbody>
                    @foreach($wallets as $key => $wallet)
                        @if (\App\Customer::where('user_id',$wallet->user_id) != null && $wallet->user->user_type == 'customer')
                            <tr>
                                <td>{{ $key+1 }}</td>
                                <td>{{ $wallet->created_at }}</td>
                                <td>
                                    @if (\App\customer::where('user_id',$wallet->user_id) != null)
                                    {{ $wallet->user->name}}
                                @endif
                                </td>
                                <td>
...
<div class="modal fade" id="addOrRemove" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form action="{{route('customers.add_or_remove')}}" method="POST">
            @csrf
        ...
        <div class="form-group">
            <div class="row">
                <label class="col-sm-3 control-label" for="customer">{{__('customer')}}</label>
                <div class="col-sm-9">
                <select name="customer_id" id="" class="form-control demo-select2" onchange="customerSelect(this)">
                        @foreach (App\User::where('user_type','customer')->get() as $user)
