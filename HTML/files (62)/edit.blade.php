...
<div class="col-lg-6 col-lg-offset-3">
    <div class="panel">
        <form class="form-horizontal" action="{{ route('admin.update-shipping',$shipping->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <input name="_method" type="hidden" value="PATCH">
                <div class="panel-heading">
                    <h3 class="panel-title">{{__('Shipping Information Edit')}}</h3>
                </div>
                <div class="panel-body">
                    ...
                    <div class="col-sm-9">
                         <select name="type" class="form-control" onchange="shippingType(this)">
                           ...
                           @elseif($shipping->type=='flat_rate')
             ...
             <div style="display:{{($shipping->type=='free'||$shipping->type=='flat_rate')?'none':'block'}}"> 
                        <div class="form-group">
                            <label class="col-sm-3 control-label" for="name">{{__('Name')}}</label>
                            <div class="col-sm-9">
                                <input type="text" placeholder="{{__('Name')}}" id="name" value="{{$shipping->name}}" name="name" class="form-control">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-3 control-label" for="extra">{{__('Extra')}}</label>
                            <div class="col-sm-9">
                                <input type="text" placeholder="{{__('Extra')}}" value="{{$shipping->extra}}" id="extra" name="extra" class="form-control" >
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <label class="switch">
                                    <input type="checkbox" name="premium" id="premium"  {{$shipping->extra=='on'?'checked':''}} onchange="premium_update(this)">
                        ...
                        <div class="without_premium" id="without_premium">
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input type="url" placeholder="{{__('Url')}}" id="url" value="{{$shipping->url}}" name="url" class="form-control">
                                </div>
                            </div>
                        </div>
                       </div> 
                        ...
                        <div id="non-premium" class="mb-5">
                            @if(!$shipping->premium)
                                @foreach(json_decode($shipping->date) as $key=>$date)
                                <div class="border">
                                    No # {{$key+1}}
                                    <div class="form-group">
                                        <label class="col-sm-3 control-label" for="date">{{__('Dates')}}</label>
                                        <div class="col-sm-9">
                                            <select name="date[]" class="form-control" id="date--{{$key}}" required>
                                                <option {{$date=="Mon"?'selected':''}} value="Mon">Monday</option>
                                                <option {{$date=="Tue"?'selected':''}} value="Tue">Tuesday</option>
                                                <option {{$date=="Wed"?'selected':''}} value="Wed">Wednesday</option>
                                                <option {{$date=="Thu"?'selected':''}} value="Thu">Thursday</option>
                                                <option {{$date=="Fri"?'selected':''}} value="Fri">Friday</option>
                                                <option {{$date=="Sat"?'selected':''}} value="Sat">Saturtday</option>
                                                <option {{$date=="Sun"?'selected':''}} value="Sun">Sunday</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="shipped[]" class="form-control" id="shipped--{{$key}}" required>
                                                <option  value="same_date">Same Date</option>
                                                <option  {{json_decode($shipping->shipped)[$key]=="Mon"?'selected':''}} value="Mon">Monday</option>
                                                <option  {{json_decode($shipping->shipped)[$key]=="Tue"?'selected':''}} value="Tue">Tuesday</option>
                                                <option  {{json_decode($shipping->shipped)[$key]=="Wed"?'selected':''}} value="Wed">Wednesday</option>
                                                <option  {{json_decode($shipping->shipped)[$key]=="Thu"?'selected':''}}  value="Thu">Thursday</option>
                                                <option  {{json_decode($shipping->shipped)[$key]=="Fri"?'selected':''}} value="Fri">Friday</option>
                                                <option  {{json_decode($shipping->shipped)[$key]=="Sat"?'selected':''}} value="Sat">Saturtday</option>
                                                <option  {{json_decode($shipping->shipped)[$key]=="Sun"?'selected':''}} value="Sun">Sunday</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-9">
                                            <input type="time" id="at--{{$key}}"  value="{{json_decode($shipping->at)[$key]}}" name="at[]" required class="form-control">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-3 control-label" for="day_by">{{__('Delivery Time')}}</label>
                                        <div class="col-sm-4">
                                            <input type="number" value="{{json_decode($shipping->day_by)[$key]}}" placeholder="{{__('Day by')}}" id="day_by--{{$key}}" name="day_by[]" class="form-control">
                                        </div>
                                        <label class="col-sm-1 control-label" for="day_to">{{__('#')}}</label>
                                        <div class="col-sm-4">
                                            <input type="number" value="{{json_decode($shipping->day_to)[$key]}}" placeholder="{{__('Day to')}}" id="day_to--{{$key}}" name="day_to[]" class="form-control">
                                        </div>
                                    </div>
                                </div>
