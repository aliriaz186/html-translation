                ...
                <div class="row no-gutters cols-xs-space cols-sm-space cols-md-space">
                    ...
                    <div class ={{(isset(App\ads::first()->secondAd) || isset(App\ads::first()->thirdAd) || App\ProductBuyerTips::where('status',1)->first())? "col-lg-5" :"col-lg-6"}}>
                        ...
                        <div class="product-description-wrapper">
                            ...
                            @endphp
                            @if(home_price($detailedProduct->id) != home_discounted_price($detailedProduct->id) && ($today->greaterThan($nintyDayPassed) && (App\OrderDetail::where('product_id',$detailedProduct->id)->count()>20)) || $detailedProduct->wholesale)
                                ...
                                <div class="row no-gutters mt-3">
                                    ...
                                    <div class="col-8">
                                        <div class="product-price">
                                            <strong id="home_discounted_price">
                                                {{ home_discounted_price($detailedProduct->id) }}
                                            </strong>
                                           @if($detailedProduct->wholesale)
                                            <span class="piece" id="discount_value">/{{ json_decode($detailedProduct->wholesale_discount)[0] }}{{_('% Discount')}}</span>
                                            @else
                            ...
                            <br>
	                            ...
	                            <div class="col-2">
	                                <div class="product-description-label">{{__('Wholesale Discount:')}}</div>
	                            </div>
	                            <div class="col-5">
	                                <div class="text-success font-weight-bold">
	                                 <select class="form-control selectpicker" data-placeholder="Select a discount" onchange="select_discount(this)">
                                        @foreach(json_decode($detailedProduct->min) as $key=>$min)
                                            <option value="{{$min}}">{{$min}} - {{json_decode($detailedProduct->max)[$key]}}</option>
                                        @endforeach
                        ...
                        <div class="row no-gutters mt-1">
                               ...
                               @foreach(App\Cashback::where('enable',1)->get() as $cashback)
                                 @foreach(json_decode($cashback->products) as $product)
	                               ...
	                               <div class="col-8">
	                                   @php
                               			$discount = explode(currency_symbol(), home_discounted_price($detailedProduct->id));
...
function search(){
  var search = $('#search_query').val();
   ...
   function showCheckoutModal(){
    ...
    function select_discount(el){
    	 $.post('{{ route('products.discount') }}',{_token:'{{ csrf_token() }}', min:el.value , id:{{$detailedProduct->id}}}, function(data){
                            main_price = '{{home_discounted_price($detailedProduct->id,0)}}';
                            main_price = main_price.split("{{currency_symbol()}}")[1];
                            $('#discount_value').html('/'+data+'% Discount');
                            price = '{{currency_symbol()}}'+((parseFloat(main_price)*(100-parseInt(data)))/100).toFixed(2);
                            $('#home_discounted_price').html(price);
                        });
    }
</script>
@endsection
