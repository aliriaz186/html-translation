...
<div class="modal-body gry-bg px-3 pt-0">
    ...
    <ul class="nav nav-tabs mt-3" id="myTab" role="tablist">
        ...
        @foreach ( $order->orderDetails->where('seller_id', Auth::user()->id) as $orderDetail)            
          <li class="nav-item"><a class="nav-link active" id="{{$orderDetail->product->name}}-tab" data-toggle="tab" href="#all" role="tab" aria-controls="{{$orderDetail->product->name}}" aria-selected="true">{{$orderDetail->product->name}}</a></li>
        @endforeach
    </ul>
    <div class="tab-content" id="myTabContent">

        @foreach ( $order->orderDetails->where('seller_id', Auth::user()->id) as $i=>$orderDetail)
           @php $id = $orderDetail->product->name @endphp  
        <div class="tab-pane fade  @if($i==0) show active @endif" id="{{$id}}" role="tabpanel" aria-labelledby="{{$id}}-tab">
            <div class="contaienr">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card mt-4">
                            <div class="card-header py-2 px-3 heading-6 strong-600">{{__('Shipping Information')}}</div>
                            ...
                            @if($tracker_number!='')
                                <table class="table table-bordered table-sm table-hover table-responsive-md">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                        <button  data-toggle="modal"  onclick="ship_order({{$seller_order_details->id}})" data-target="#shipOrder" class="btn btn-warning w-50 text-white" >Edit</button>
                                        <button class="btn btn-primary custom-block" style="width:48%"  id="tracker_submit">Track order</button>
                                        </td>
                                        </tr>
                                        </tbody>
                                    </table>
                                    @else
                                    <form method="post" action="{{route('seller.set.courier')}}">
                                    @csrf
                                    <table class="table table-bordered table-sm table-hover table-responsive-md">
                                        <thead>
                                        <tr>
                                    ...
                                    <td>
                                        ...
                                        <div class="form-group">
                                            <select name="shipping_courier_type" class="form-control courier_shipping_type" required>
                                                <option value="" selected>Please select shipping method</option>
                                                    @foreach (App\Shipping::all() as $ship)
                                                        <option value="{{$ship->id}}">{{ ucfirst(str_replace('_',' ',$ship->name))}} @if($ship->premium=="on") (Premium) @endif</option>
                                                    @endforeach
                                            </select>
                                        </div>
                                        </div>
                                        ...
                                        <td>
                                        <input type="number" name="track_number" class="form-control" placeholder="Tracking Number">
                                        <input type="hidden" name="id" class="form-control" value="{{$orderDetail->id}}">
                                        </td>
                                        <td>
                                        <input type="submit" class="btn btn-primary w-100 text-white" value="Set">
                                        </td>
                                        </tr>
                                        <tbody>
                                        </table>
