        ...
        <div class="px-2 py-4 p-md-4 bg-white shadow-sm">
            <div class="section-title-1 clearfix">
                <h3 class="heading-5 strong-700 mb-0 float-left">
                    <span class="mr-4">{{__('Recently Viewed Products')}}</span>
                </h3>
                <ul class="inline-links float-right">
                    <li><a  href="{{route('cookies-products')}}" class="active">{{__('Show More')}}</a></li>
                </ul>
            </div>
            <div class="caorusel-box arrow-round gutters-5">
                <div class="slick-carousel" data-slick-items="6" data-slick-xl-items="5" data-slick-lg-items="4"  data-slick-md-items="3" data-slick-sm-items="2" data-slick-xs-items="2">

                   @php $products_ids = Cookie::get('user_viwed_products'); @endphp
                   @foreach (json_decode($products_ids) as $ids)
                        @php $product = App\Product::findOrFail($ids); @endphp
                        <div class="caorusel-card my-1">
                            <div class="row no-gutters product-box-2 align-items-center">
                                ...
                                <div class="col-8 border-left">
			                     ...
			                     <div>
			                       <span style="left:45%" class="product-label label-hot">{{__('Free Returns')}}</span>
			                    </div>
				    @endif
                                        <h2 class="product-title mb-0 p-0 text-truncate-2">
                                            <a href="{{ route('product', $product->slug) }}">{{ __(Illuminate\Support\Str::limit($product->name, 10) ) }}</a>
                                        </h2>
