...
@section('content')
                                                ...
                                                <div class="order_detail_track">
                                                    <div class="row">
                                                        <div class="col-md-4 col-sm-4 col-xs-12">
                                                            <span class="ml-2 mr-3 font-weight-bold cursor" style="font-size:1rem" type="button" data-toggle="collapse" data-target="#detailed--{{$id}}--{{$key}}" aria-expanded="false" aria-controls="detailed"><i class="fa fa-plus"></i> <i class="fa fa-minus"></i>  </span>
                                                            <button class="btn btn-primary" onclick="show_order_details({{ $order->id }})" href="#{{ $order->code }}" >{{ $order->code }}</button> 
                                                            @if (\App\RequestsNotification::where(['type' => 'refund', 'seller_id' => $orderDetails->seller->id,'order_id'=>$orderDetails->id])->exists())
                                                                <span class="ml-2 text-success font-weight-bold">(New)</span>
                                                            @endif                         
                                                        </div>
                                                        <div class="col-md-4 col-sm-4 col-xs-12">
                                                        ...
                                                        <div class="col-md-12 details_part_product_img slingle_item_address_part">
                                                            <div class="row">
                                                                ...
                                                                <div class="col-md-4 col-sm-6 col-xs-8">
                                                                    <p>
                                                                        <a href="{{route('product',$orderDetails->product->slug)}}" target="_blank" title="{{$orderDetails->product->name}}" class="font-weight-bold">{{ $orderDetails->product->name}} @if($order->delivery_viewed == 0 && $order->payment_status_viewed == 0)<span class="ml-2 text-success"><strong>({{ __('New') }})</strong></span>@endif  </a><br>
                                                                        {{__('Price:')}} {{home_discounted_price($orderDetails->product->id)}}<br> 
                                                                        {{__('Qty:')}} {{$orderDetails->quantity}}<br>
                                                                        {{__('SKU:')}} {{$orderDetails->product->sku}}<br>
                                                                        {{__('Variations:')}} {{$orderDetails->product->variation==''?'NAN':$orderDetails->product->variation}}
                                                                    </p>
                                                                </div>
                                                                <div class="col-md-4 col-sm-4 col-xs-12 no-print">
                                                                    <p class="font-weight-bold">
                                                                        ...
                                                                        <span class="font-weight-bold">{{ ucfirst(str_replace('-',' ',$orderDetails->refund_request)) }}</span><br>                                                                    </p>    
                                                                ...
                                                                <div class="col-md-3 col-sm-7 col-xs-12 col-md-offset-5 text-right">
                                                                       @if($status == 'Authorised Request' )
                                                                            <button class=" btn btn-block btn-success mb-1"  onclick="refundOrderApproveModal({{$order->id}},{{$order->orderDetails->where('seller_id', Auth::user()->id)->first()->product->unit_price}},'approve')" data-toggle="modal" data-target="#refundOrderRequest">Accept</button>
                                                                            <button class="btn btn-block btn-danger mb-1" onclick="refundOrderRejectModal({{$order->id}},'reject')" data-toggle="modal" data-target="#refundOrderRequest">Reject</button>
                                                                       @elseif($status == 'Unauthorised Request' || $status == 'Processing Refund' || $status == 'Cancelled Request' || $status == 'Cancelled Return')
                                                                       ...
                                                                       @else
                                                                            <button class="btn btn-block btn-success mb-1" onclick="refundOrderAuthroizedModal({{$order->id}},'authroised')" data-toggle="modal" data-target="#refundOrderAuthroized">Authroised</button>
                                                                            <button class="btn btn-block btn-danger mb-1" onclick="refundOrderRejectModal({{$order->id}} , 'unauthroised')" data-toggle="modal" data-target="#refundOrderRequest">Unauthroised</button>   
                                                                        @endif
                                                                </div>
                                                            </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div id="isApproveType" style="display: none">
                            <div class="form-group">
                                <select class="form-control" required name="refund_request" id="refundrequest" onchange="selectRefund(this)">
    ...
    <div class="modal fade" id="order_details" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal fade" id="refundOrderAuthroized" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <script>
       ...
       function refundOrderAuthroizedModal(id,type) {
        ...
        function selectRefund(el){
        	if(el.value == 'Full Refund'){
        		$('#refundAmount').val($('#grand_total').val());
