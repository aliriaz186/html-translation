                            ...
                            <div class="col-md-6">
                                <div class="float-md-right">
                                    <ul class="breadcrumb">
                                            ...
                                            <li class="active"><a href="{{ route('seller.set_shipping_profile') }}">{{__('Shipping Countries Profile')}}</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                                 ...
                                 <div class="card no-border mt-4" style="width:100%">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <form action="{{route('seller.edit_default_shipping_price')}}" method="POST">
                                                ...
                                                <ul class="nav nav-tabs" role="tablist">
                                                    ...
                                                    @foreach(json_decode(App\SellerCountry::where('seller_id',Auth::user()->id)->first()->setCountries) as $country_key=>$country)
                                                        <li class="nav-item">
                                                            ...
                                                            <a class="nav-link {{$country_key==0?'active':''}}" data-toggle="tab" href="#tabs-{{$country_key}}" role="tab">{{$country}}</a>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                                    ...
                                                    <div class="tab-pane {{$country_key==0?'active':''}}" id="tabs-{{$country_key}}" role="tabpanel">
                                                        <div class="row">
                                                                ...
                                                                <div class="col-md-8">
                                                                        ...
                                                                        if($country_key < count(json_decode($shh->default_courier_company))){
                                                                        ...
                                                                        <select name="shipping_type[]" onchange="changeCatch('{{$country}}',this)" class="form-control" disabled>
                                                            ...
                                                            <div class="row"  id="{{$country}}--flat_shipping_data" @if ($shipping_type_selected=='flat_rate') style="display:flex" @else style="display: none" @endif>
                                                                ...
                                                                <div class="col-md-8">
                                                                    <input type="number" min="0" step="0.01"  class="form-control mb-3" readonly name="shipping_flat_price[]" placeholder="{{__('Flat Rate Cost')}}" @if ($shipping_type_selected=='flat_rate') value="{{$cost_shipping}}" @endif>
                                                                </div>
                                                                <div class="col-md-2">
                                                                    <label class="switch" style="margin-top:5px;">
                                                                        <input type="radio" name="shipping_flat_type" value="flat_rate" checked  onclick="return false;">
                                                        ...
                                                        <div class="row" id="{{$country}}--free_shipping_data"  @if ($shipping_type_selected=='free') style="display:flex" @else style="display: none" @endif>
                                                            ...
                                                            <div class="col-md-8">
                                                                <input type="number" min="0" step="0.01" value="0" class="form-control mb-3" name="shipping_free_price"  disabled placeholder="{{__('FREE Cost')}}">
                                                            </div>
                                                            <div class="col-md-2">
                                                                <label class="switch" style="margin-top:5px;">
                                                            ...
                                                            <div class="row"  @if ($shipping_type_selected!='courier') style="display: none" @endif >
                                                                @php $ship_courier_company_id_with_country = explode('--',$ship_courier_company_id_with_country);  $get_id_courier_company = $ship_courier_company_id_with_country[0]; $get_country = $ship_courier_company_id_with_country[1];  $courier_company_prices = explode('--',$shippingCourierPrice[$key]); @endphp
                                                                    @if($country == $get_country)
                                                                        ...
                                                                        <div class="col-md-8">
                                                                            <select name="shipping_courier_type[]" class="form-control demo-select2" onchange="premiumChange('{{$country}}',this)" disabled>
                                                                        ...
                                                                        <label class="switch mt-1 {{$get_country}}_dafault_company_label "   data-toggle="tooltip" title="set it default company">
                                                                            <input type="checkbox"  class="{{$get_country}}_dafault_company" @if($shippingCourierDefault) {{ $shippingCourierDefault[0] ==''.$get_country.'__'.$courier_company->id?'checked':''}}@endif  onclick="return false"  value="{{$get_country}}_default" name="default_company[]" >
                                                                            <span class="slider round"></span>
                                                                        </label>
                                                                    @endif
                                                            </div>
                                                            ...
                                                            @if($get_country == $country)
                                                                <div class="row" id="{{$country}}--premium_price{{$key}}"   style="{{$courier_company->premium == 'on'?'display: none':'display:flex' }}">
                                                                    ...
                                                                    <div class="col-md-8 mt-3">
                                                                        <input  type="number" min="0" step="0.01" value="{{$courier_company_prices[0]}}" class="form-control mb-3" name="shipping_courier_price[]" placeholder="{{__('Price')}}" @if ($shipping_type_selected=='courier') value="{{$cost_shipping}}"  @endif readonly disabled>
                                                                    </div>
                                                                </div>
                                                                <div class="row">
                                                        ...
                                                        @else
                                                            <div id="{{$country}}--courier_shipping_data" style="display:none">
                                                                ...
                                                                <div class="row" id="{{$country}}--premium_price1">
                                                                    ...
                                                                    <div class="col-md-8">
                                                                        <input  type="number" min="0" step="0.01" value="0" class="form-control mb-3" name="shipping_courier_price[]" placeholder="{{__('Price')}}" readonly disabled>
                                                                    </div>
                                                                </div>
                                                                <div class="row">
                                            ...
                                            <a  href="{{route('countries.edit_default_countries')}}" class="btn btn-sm btn-primary pull-right">Edit Profile</a>
                                        </form>
                                        </div>
                                    </div>
                                </div>
