                    ...
                    <div class="main-content">
                        ...
                        <form class="" action="{{ route('customer.profile.update') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="footer-top-box text-center bg-white">
                                <a href="#">
                                    @if(Auth::user()->decide_type == 2)
                                    <label class="switch" data-toggle="tooltip" title="Seller can't disable customer Account unless become a Customer">
                                        <input type="checkbox" onclick="return false;"/>
                                        <span class="slider round"></span>
                                    </label>
                                    @else
                                    <label class="switch" data-toggle="tooltip" title="to delete your account, click on the information icon below">
                                        <input type="checkbox" onchange="updateDisabledByCustomer( '{{Auth::user()->id}}')" {{Auth::user()->disabled_by_user == 1?'checked':''}} />
                                        <span class="slider round"></span>
                                    </label>
                                    @endif
                            ...
                            <div class="form-box bg-white mt-4">
                                <div class="form-box-title px-3 py-2">
                                    {{__('Basic info')}}
                                           <p class="font-weight-bold pull-right">Registered as a customer on  <span >  {{Auth::user()->customer->created_at->format('d-m-Y @ h:i:s')}} </span> </p>
                                </div>
                            ...
                            <div class="form-box bg-white mt-4">
                                ...
                                <div class="form-box-content p-3">
                                   <div class="row">
                                        ...
                                        <div class="col-md-10">
                                            <textarea class="form-control textarea-autogrow mb-3" placeholder="{{__('Your Name')}}" rows="1" name="name">{{Auth::user()->name}}</textarea>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-2">
                            ...
                            <div class="text-right mt-4">
                                ...
                                <button type="submit" id="update_btn" class="btn btn-styled btn-base-1" >{{__('Update Profile')}}</button>
                            </div>
                        </form>
                    </div>
                </div>
...
function updateDisabledByCustomer(user_id){
    	    $.get('{{ route('customer.disabled_by_user_update') }}',{_token:'{{ @csrf_token() }}', user_id:user_id}, function(data){
