...
@if($type != 'Seller' && (permission_check_all('inhouse_products') || permission_check_post('inhouse_products')))
    <div class="row">
                ...
                @foreach($products as $key => $product)
                    <tr>
                        ...
                        <td><label class="switch">
                          ...
                          <td><label class="switch">
                                <input onchange="update_edit(this)" value="{{ $product->id }}" type="checkbox" <?php if($product->is_edit_seller== 1) echo "checked";?> >
                                <span class="slider round"></span></label></td>
                        <td>
                            <div class="btn-group dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    @if(permission_check_all('inhouse_products') || permission_check_put('inhouse_products') ||
                                    permission_check_all('products') || permission_check_put('products') )
                                        @if ($type == 'Seller')
                                        ...
                                        @endif
                                    @endif
                                    @if(permission_check_all('inhouse_products') || permission_check_delete('inhouse_products') ||
                                   permission_check_all('products') || permission_check_delete('products')
                                   )
                                        <li><a onclick="confirm_modal('{{route('products.destroy', $product->id)}}');">{{__('Delete')}}</a></li>
                                    @endif
                                    @if(permission_check_all('duplicate') || permission_check_post('duplicate'))
...
@section('script')
    <script type="text/javascript">
        ...
        function update_edit(el){
            ...
            $.post('{{ route('products.edit.seller') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showAlert('success', 'Edit products updated successfully');
                }
                else{
                    showAlert('danger', 'Something went wrong');
                }
