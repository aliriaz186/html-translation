...
<div class="panel">
    <div class="panel-heading bord-btm clearfix pad-all h-100">
        <h3 class="panel-title pull-left pad-no">{{__('Permissions Request')}}</h3>
        ...
        <table class="table table-striped res-table mar-no" cellspacing="0" width="100%">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{__($permission->category->name)}}</td>
                    <td>{{__($permission->seller->email)}}</td>
                    <td><img loading="lazy"  class="img-xs" src="{{ asset($permission->category->icon) }}" alt="{{__('icon')}}"></td>
                    ...
                    <td>
                        <div class="btn-group dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                @if(permission_check_all('permission_sellers') || permission_check_delete('permission_sellers'))
                                    <li><a onclick="confirm_modal('{{route('categories.permission.delete', $permission->id)}}');">{{__('Delete')}}</a></li>
                                @endif
                                </ul>
                        </div>
                    </td>
...
@section('script')
  <script>
        function update_permission(el){
            ...
            $.get('{{ route('categories.permission.status') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
