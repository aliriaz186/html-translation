        ...
        <table class="table table-striped res-table mar-no" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <th>{{__('Banner')}}</th>
                    <th>{{__('Icon')}}</th>
                    @if(permission_check_all('categories') || permission_check_post('categories') )<th>{{__('Featured')}}</th>@endif
                    <th>{{__('Commission')}}</th>
                    @if(permission_check_all('categories') || permission_check_post('categories') )<th>{{__('Permission')}}</th>@endif
...
@section('script')
    <script type="text/javascript">
        ...
        function update_permission(el){
            ...
            $.post('{{ route('categories.permission') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
